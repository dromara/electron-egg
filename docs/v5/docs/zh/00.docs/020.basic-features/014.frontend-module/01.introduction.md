...
只需一个文件：[ipcRenderer.js](/00.docs/020.basic-features/040.communication/020.ipc-render)
...
###  开发环境
可同时启动 前端 & 主进程，也可以分开启动, [快速开始](/00.docs/010.getting-started/040.quick-start)
...
::: tip
注1：如果启动的前端服务不是：http://localhost:8080/  请先配置：[cmd/bin.js](/00.docs/010.getting-started/050.config/01.dev-config) 文件中的 `frontend` 对象
...
把 `项目部署的基础路径` 改为 `相对路径`，或 能被访问到的路径（可用 [开发者工具栏](/00.docs/010.getting-started/050.config/02.base-config#开发者工具) 查看资源路径是否正确）
