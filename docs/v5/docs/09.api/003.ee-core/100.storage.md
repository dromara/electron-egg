...
**参见**：[Types](/09.api/003.ee-core/115.types) for `Config.storage` related types
...
| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `name` | `string` | 是 | Database name or path. Supports four formats (see modes below) |
...
**返回值**：`SqliteStorage` instance (without `db` property set).
...
| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `opt` | `Record<string, unknown>` | 否 | better-sqlite3 database options. Default merged with `{ timeout: 5000 }` |

**返回值**：`Promise<this>` — Returns the SqliteStorage instance for chaining. After resolution, `this.db` is available.
...
| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `name` | `string` | 是 | Database name or path |

**返回值**：`string` — `'memory'`, `'onlyName'`, `'relative'`, or `'absolute'`.

### getDbDir()

Get the absolute path of the directory containing the database file.

**返回值**：`string` — Database directory path.
...
**返回值**：`string` — Full database file path.
