...
**描述**: Inter-process communication channel constants. Used by the main process and renderer process for Electron IPC messaging. These channel names follow the `ee#` prefix convention to avoid conflicts with user-defined channels.
...
**描述**: SocketIO communication channel constants. Used by the Socket.IO server for third-party process communication (Go/Python backends, etc.). The `partySoftware` value is the default channel name in the framework's Socket.IO configuration.
...
**描述**: Process lifecycle event constants. Used by the framework's exception handling module and child process management to identify specific process exit and error events.
...
**描述**: Message receiver type constants. Used in IPC message routing to identify the intended recipient of a message. Determines how the framework dispatches incoming messages from child processes.
