---
title: ehelper
---

## Introduction

ehelper 是 ee-go 的工具函数模块，提供常用的辅助功能，包括 JSON 返回值构造、文件/目录检测、随机字符串生成、字符串包含判断、切片去重、Map 合并、随机延时和随机整数生成等。这些函数覆盖了应用开发中最常见的基础操作，减少重复代码编写。

## Import

```go
import "github.com/wallace5303/ee-go/ehelper"
```

## API

### GetJson()

**描述**: 创建一个标准的 JSON 返回值结构体，默认 `Code` 为 `0`，`Msg` 为空字符串，`Data` 为 `nil`。用于 HTTP 接口的统一响应格式。

**参数**: 无

**返回值**: `*ResultJson` — 初始化后的 JSON 结果结构体指针

**示例**:

```go
result := ehelper.GetJson()
result.Code = 0
result.Msg = "success"
result.Data = map[string]any{
    "id":   1,
    "name": "example",
}
// 返回给前端
c.JSON(200, result)
```

**参见**: `ResultJson`

---

### ResultJson

**描述**: 统一的 JSON 响应结构体，用于 HTTP 接口返回数据。包含状态码、消息和数据三个字段，字段名通过 JSON tag 映射为小写。

| 字段 | 类型 | JSON Tag | 说明 |
|------|------|----------|------|
| Code | `int` | `"code"` | 状态码，0 表示成功 |
| Msg | `string` | `"msg"` | 消息内容，成功时可为空 |
| Data | `any` | `"data"` | 返回数据，可为任意类型 |

**示例**:

```go
// 自定义响应
r := &ehelper.ResultJson{
    Code: -1,
    Msg:  "parameter error",
    Data: nil,
}
```

---

### FileIsExist(path)

**描述**: 检查指定路径的文件或目录是否存在。如果路径存在返回 `true`，不存在返回 `false`。

**参数**:

| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| path | `string` | 是 | 文件或目录的绝对/相对路径 |

**返回值**: `bool` — 文件存在返回 `true`，不存在返回 `false`

**示例**:

```go
configPath := "/opt/app/config.json"
if ehelper.FileIsExist(configPath) {
    // 读取配置
} else {
    // 使用默认配置
}
```

---

### IsDir(path)

**描述**: 检查指定路径是否为目录。如果路径不存在或不是目录，返回 `false`。

**参数**:

| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| path | `string` | 是 | 待检查的路径 |

**返回值**: `bool` — 是目录返回 `true`，不是目录或路径不存在返回 `false`

**示例**:

```go
path := "/opt/app/data"
if ehelper.IsDir(path) {
    fmt.Println("data is a directory")
} else {
    os.MkdirAll(path, 0755)
}
```

**参见**: `FileIsExist()`

---

### GetRandomString(length)

**描述**: 生成指定长度的随机字符串，字符范围包含小写字母 `a-z` 和数字 `0-9`。每次调用之间有 10ns 的间隔以保证随机性。

**参数**:

| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| length | `int` | 是 | 生成字符串的长度 |

**返回值**: `string` — 包含小写字母和数字的随机字符串

**示例**:

```go
// 生成 16 位随机字符串
token := ehelper.GetRandomString(16)
fmt.Println("Token:", token) // 例如: "a3b7c9d2e5f8g1h4"
```

---

### Contains(str, strs)

**描述**: 判断字符串 `str` 是否存在于字符串切片 `strs` 中。逐个比较，找到匹配即返回 `true`。

**参数**:

| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| str | `string` | 是 | 待查找的字符串 |
| strs | `[]string` | 是 | 目标字符串切片 |

**返回值**: `bool` — 存在返回 `true`，不存在返回 `false`

**示例**:

```go
events := []string{"beforeClose", "ready"}
if ehelper.Contains("beforeClose", events) {
    fmt.Println("event is supported")
}
```

**参见**: `SlicesContains()`

---

### SlicesContains(slice, str)

**描述**: 判断字符串 `str` 是否存在于字符串切片 `slice` 中。功能与 `Contains()` 一致，命名风格不同。框架内部统一使用此函数。

**参数**:

| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| slice | `[]string` | 是 | 目标字符串切片 |
| str | `string` | 是 | 待查找的字符串 |

**返回值**: `bool` — 存在返回 `true`，不存在返回 `false`

**示例**:

```go
validEvents := []string{"beforeClose"}
if ehelper.SlicesContains(validEvents, eventName) {
    // 注册事件
}
```

**参见**: `Contains()`

---

### RemoveDuplicatedElem(slice)

**描述**: 去除字符串切片中的重复元素，保留首次出现的元素，返回去重后的新切片。原切片不被修改。

**参数**:

| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| slice | `[]string` | 是 | 待去重的字符串切片 |

**返回值**: `[]string` — 去重后的字符串切片

**示例**:

```go
names := []string{"alice", "bob", "alice", "charlie", "bob"}
unique := ehelper.RemoveDuplicatedElem(names)
fmt.Println(unique) // ["alice", "bob", "charlie"]
```

---

### Mapserge(src, tgt, itgt)

**描述**: 将源 Map `src` 合并到目标 Map `tgt` 中。对于目标 Map 中已存在的键，若值为 Map 类型则递归合并，否则用源值覆盖目标值。键名匹配大小写不敏感。可选的 `itgt` 参数（`map[any]any` 类型）会同步接收合并结果，用于兼容 Viper 内部的 `map[any]any` 类型数据。

**参数**:

| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| src | `map[string]any` | 是 | 源 Map，提供要合并的键值对 |
| tgt | `map[string]any` | 是 | 目标 Map，接收合并结果 |
| itgt | `map[any]any` | 否 | 可选的中间 Map，同步接收合并结果；传 `nil` 可忽略 |

**返回值**: 无（直接修改 `tgt` 和 `itgt`）

**示例**:

```go
// 配置合并：将环境配置合并到默认配置
defaultCfg := map[string]any{
    "http": map[string]any{
        "port":   8080,
        "enable": true,
    },
}
envCfg := map[string]any{
    "http": map[string]any{
        "port": 9090,
    },
}
ehelper.Mapserge(envCfg, defaultCfg, nil)
// defaultCfg["http"]["port"] 被覆盖为 9090
// defaultCfg["http"]["enable"] 保留为 true
```

---

### RandomSleep(minMills, maxMills)

**描述**: 在指定的时间范围内随机休眠。休眠时间为 `[minMills, maxMills)` 毫秒之间的随机值。适用于避免并发请求集中、模拟延迟等场景。

**参数**:

| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| minMills | `int` | 是 | 最小休眠时间（毫秒） |
| maxMills | `int` | 是 | 最大休眠时间（毫秒） |

**返回值**: 无

**示例**:

```go
// 随机休眠 100-500 毫秒
ehelper.RandomSleep(100, 500)
```

**参见**: `Int()`

---

### Int(min, max)

**描述**: 生成 `[min, max)` 范围内的随机整数。每次调用之间有 10ns 的间隔以保证随机性。

**参数**:

| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| min | `int` | 是 | 随机数下界（包含） |
| max | `int` | 是 | 随机数上界（不包含） |

**返回值**: `int` — 范围内的随机整数

**示例**:

```go
// 生成 1-100 的随机数
num := ehelper.Int(1, 100)
fmt.Println("Random number:", num)
```

**参见**: `RandomSleep()`
