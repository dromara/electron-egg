...
### Introduction
CLI module for development, build, encryption, packaging, and resource management. Dual CJS + ESM module support.
...
|--------|-----------|-------------|
| `--config` | `<folder>` | Path to custom bin.js config file |
| `--flag` | `<flag>` | Comma-separated move flags (e.g. `frontend_dist`) |
