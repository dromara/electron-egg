...
// 获取bundle文件路径
function getBundleDir(basePath?: string) {
...
// 获取electron 源码文件路径
function getElectronCodeDir(basePath?: string) {
  const base = basePath || process.cwd();
...
// 获取frontend 源码文件路径
function getFrontendCodeDir(basePath?: string) {
  const base = basePath || process.cwd();
...
// 退出ChildJob进程
function exitChildJob(code: number = 0) {
  try {
...
function getArgumentByName(name: string, args?: string[]) {
  if (!args) {
    args = process.argv;
  }
